...
public class LoginTestStepDefinition extends Base {
	...
	public void user_should_see_logout_message(String message) throws Throwable {
	    
		Assert.assertTrue(myAccountPageObj.isLogoutMessageOneDisplayed());
		WebDriverUtility.screenShot();
		WebDriverUtility.wait(5000);
	}

	@And("^User should see second logout message '(.+)'$")
	public void user_should_see_second_logout_message(String message) throws Throwable {
		
		Assert.assertTrue(myAccountPageObj.isLogoutMessageTwoDisplayed());
		WebDriverUtility.screenShot();
		WebDriverUtility.wait(5000);
	}
